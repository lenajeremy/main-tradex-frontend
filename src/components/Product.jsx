...
const Product = ({productDetails, view, manage, self}) =>{
  ...
  return(
    <motion.div key ={productDetails.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1}} exit={{ opacity: 0.5, scale: 0.5 }} className = 'product'>
      ...
      {self ? <Close onClick = {()=>manage(productDetails)}/> : ''}
      ...
      <div className="content">
        ...
        <button className = 'btn btn-danger btn-sm' onClick = {()=>manage(productDetails, 'add to cart')}>{view === 'store' ? 'Remove from Store' : self ? "Remove Product" : "Add to Cart"}</button>
