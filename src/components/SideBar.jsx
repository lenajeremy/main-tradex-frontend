    ...
    <div className='sideBar'>
      <div className='links'>
        ...
        <div className={`sideBar__links left ${active === 'account' ? 'active' : ''}`}>
            <div className='top'></div>
            <div className='bottom'></div> 
          <Link onClick = {() => dispatch(editSidebar({ value: 'account' }))} to={userDetails.id ? `/user/${userDetails.id}/account` : '/login' }>{active === 'account' ? <Mail/> :<MailOutline/>}<motion.span className = 'text'>Chats</motion.span></Link>
        </div>
        {userTypeDeterminant()}
